...
const LocationSelector = ({ onClose }: { onClose: () => void }) => {
  ...
  const handleGetCurrentLocation = () => {
    ...
    // Mock geolocation to avoid actual browser geolocation permissions in the demo
    setTimeout(() => {
      const currentLocation = {
        name: "Current Location, Indiranagar, Bangalore",
        coordinates: { lat: 12.9719, lng: 77.6412 }
      };
    ...
    if (navigator.geolocation) {
      navigator.geolocation.getCurrentPosition(
        (position) => {
          ...
          // Here we would reverse geocode to get the address
          const location = {
